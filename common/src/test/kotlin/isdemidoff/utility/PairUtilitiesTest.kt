...
class PairUtilitiesTest : FreeSpec({
    "Check CharSequence#keyValue(String)" - {
        withData(
            nameFn = { "'${it.inputString}' delimited by '${it.delimiter}' should result into ${it.result}" },
        ...
        withData(
            nameFn = { "'${it.inputString}' delimited by '${it.delimiter}' should throw an exception" },
...
private data class KeyValueTestData(
...
private data class KeyValueByTestData<K, V>(
    val inputString: String,
    val delimiter: String,
